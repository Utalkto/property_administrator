...
from .query_serializers import PropertyAPIQuerySerializer
...
class PropertyAPI(APIView):
    ...
    400: "'error': 'ValueError: the value of client_id is not valid it must be int or set to all'",
    401: " 'error': 'Invalid access'" })
    def get(self, request,  client_id):
        """Obtiene Propiedad(es)

        descripción: Retorna(lista) las propiedades que pertenecen a un usuario.
    ...
    responses={200: PropertySerializer(), 400: "'error': property_serializer.errors" })
    ...
    request_body=PropertySerializer(),
    responses={
    200: PropertySerializer(), 404: "'error': 'Property.DoesNotExist: the property with that id does not exist'",
    400: "'error': f'KeyError: property_id must be provided'",
    400: "KeyError: property_id must be provided"})
    ...
    @swagger_auto_schema(
    query_serializer=PropertyAPIQuerySerializer(),
    responses={200: "The property has been deleted", 404: "error: 'Property.DoesNotExist: the property with that id does not exist",
    400: "'error': 'ValueError: property_id must be int'"})
...
class UnitsAPI(APIView):
    ...
    @swagger_auto_schema(
    responses={200: UnitRelatedFieldsSerializer(), 
    400: "error: KeyError : when get_general_info is set, unit_id must be given (set to all to return all the units a client owns)", 
    404: "'error': 'Unit.DoesNotExist: the unit with provided id does not exist',"})
    def get(self, request, client_id):

        """obtiene(lista) todas la unidades
    ...
    @swagger_auto_schema(
    responses={200: UnitSerializer(), 400: "'error': unit_serializer.errors," })
    def post(self, request, client_id):
        """Crea Unidad
    ...
    @swagger_auto_schema(
    responses={200: UnitSerializer(), 400: "'error': 'ValueError: unit_id must be int'",
    404:"'error': 'Unit.DoesNotExist: the unit with provided id does not exist'"})
    def put(self, request, client_id):
        """Actializar Unidad
    ...
    @swagger_auto_schema(
    responses={200: "message: The unit has been eliminated successfully", 400: "error: ValueError: unit_id must be int",
    404:"error: Unit.DoesNotExist: the unit with provided id does not exist"})
    def delete(self, request, client_id):
        """Eliminar Unidad
